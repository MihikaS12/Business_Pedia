...
def firdeletefun():
    def deletef():
        ...
        if (con):
            ...
            cur = con.cursor()
            qry = "delete from fir where name=%s or id=%s"
            data=(namee.get(),ide.get())
            cur.execute(qry,data)
            con.commit()
            if cur.rowcount != 0:
                messagebox.showinfo("Success", "FIR is deleted")
    ...
    searchwin.configure(background="dark green");
    ...
    btndel=Button(searchwin,text="delete",command=deletef)
    btndel.grid(row=20,column=35)
